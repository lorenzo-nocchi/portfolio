...
function GridCanvas() {
  ...
  useEffect(() => {
    ...
    // Grid config - OPTIMIZED FOR PERFORMANCE
    const CELL = 80;          // increased from 60 to reduce nodes
    const ATTRACT_RADIUS = 140; // reduced from 180
    const ATTRACT_STRENGTH = 5; // reduced from 28
    const LINE_COLOR_BASE = "rgba(355, 355, 355,";   // base line color
    const NODE_COLOR_BASE = "rgba(355, 355, 355,"; // base node color
    ...
    function resize() {
      ...
      // Build grid nodes at their rest positions
      nodes = [];
    ...
    function draw() {
      ...
      // Compute displaced positions for each node
      ...
      // Draw nodes (intersection points) - FEWER & LIGHTER
...
const MIN_W = 400; // Minimum width of the window
const MIN_H = 480; // Minimum height of the window
  ...
  function onResizeMouseDown(e, direction) {
    ...
    const move = (ev) => {
      ...
      if (direction.includes("w")) {
        nw = Math.max(MIN_W, start.pw - dx);
        nx = start.px + (start.pw - nw);
      }
      if (direction.includes("n")) {
        nh = Math.max(MIN_H, start.ph - dy);
        ny = start.py + (start.ph - nh);
      }
        ...
        <div className="skills-section">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {PROFILE.languages.map((l, i) => (
                <div key={i} style={{ fontSize: 12, color: "var(--text2)", display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ color: "var(--accent)" }}>🌐</span>{l}
                </div>
            ))}
            {PROFILE.interests.map((l, i) => (
                <div key={i} style={{ fontSize: 12, color: "var(--text2)", display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ color: "var(--accent2)" }}>✦</span>{l}
                </div>
            ))}
          </div>
        </div>
...
function TerminalWindow() {
  const lines = [
    ...
    { type: "gray", text: "Étudiant en Master UX/UI à l'ESD Bordeaux, actuellement en" },
      ...
      <div className="terminal-wrap">
        {lines.map((l, i) => (
            <div key={i} className="term-line">
              ...
              {l.type === "green" && <span style={{ color: "var(--accent3)" }}>{l.text}</span>}
...
function SnakeWindow() {
  ...
  return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 16, alignItems: 'center' }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text2)', fontFamily: 'var(--font-mono)' }}>
        ...
        <div style={{
          ...
          {snake.map((seg, i) => (
              <div
                  ...
                  style={{
                    ...
                    height: CELL_SIZE - 2,
                    background: i === 0
                        ? 'linear-gradient(135deg, #e05c5c, #c45fa0)'
                        : 'rgba(224, 92, 92, 0.7)',
                    borderRadius: 2,
                    boxShadow: i === 0 ? '0 0 8px rgba(224, 92, 92, 0.6)' : 'none',
                  }}
              />
          ))}
          ...
          <div
              style={{
                ...
                width: CELL_SIZE - 2,
                height: CELL_SIZE - 2,
                background: 'linear-gradient(135deg, #a855f7, #c45fa0)',
                borderRadius: '50%',
          ...
          {gameOver && (
              <div style={{
                ...
                <div style={{ color: 'var(--accent)', fontSize: 16, fontWeight: 600 }}>GAME OVER</div>
                <button
                    onClick={resetGame}
                    style={{
                      padding: '6px 14px',
                      background: 'linear-gradient(135deg, #e05c5c, #c45fa0)',
                      border: 'none',
                      color: '#fff',
                      fontSize: 12,
                      fontWeight: 600,
        ...
        <div style={{
          ...
          <button
              onClick={() => handleArrowClick({ x: 0, y: -1 })}
              style={{
                width: 40,
                height: 40,
                background: 'linear-gradient(135deg, #e05c5c, #c45fa0)',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
                onClick={() => handleArrowClick({ x: -1, y: 0 })}
                style={{
                  width: 40,
                  height: 40,
                  background: 'linear-gradient(135deg, #e05c5c, #c45fa0)',
...
const WINDOWS_DEF = [
  { id: "profil", title: "Profil", icon: "👤", initialPosFn: (vw, vh) => {
      const layout = getResponsiveDesktopLayout(vw, vh);
      return { x: layout.profil.x, y: layout.profil.y };
    }, initialSizeFn: (vw, vh) => {
      const layout = getResponsiveDesktopLayout(vw, vh);
      return { w: layout.profil.w, h: layout.profil.h };
    }, content: <ProfilWindow /> },
  { id: "projets", title: "Projets", icon: "📁", initialPosFn: (vw, vh) => {
      const layout = getResponsiveDesktopLayout(vw, vh);
      return { x: layout.projets.x, y: layout.projets.y };
    }, initialSizeFn: (vw, vh) => {
      const layout = getResponsiveDesktopLayout(vw, vh);
      return { w: layout.projets.w, h: layout.projets.h };
    }, content: <ProjetsWindow /> },
  { id: "xp", title: "Expériences & Formation", icon: "🎓", initialPosFn: (vw, vh, w, h) => ({
  ...
  { id: "terminal", title: "À Propos · Terminal", icon: "🖥️", initialPosFn: (vw, vh, w, h) => ({
  ...
  { id: "snake", title: "Snake", icon: "🐍", desktopOnly: true, initialPosFn: (vw, vh, w, h) => ({
...
function getResponsiveDesktopLayout(vw, vh) {
  ...
  if (canSplitHorizontally) {
    ...
    if (projetsW < minStartWindowW) {
      projetsW = minStartWindowW;
      profilW = availableW - projetsW;
    }
        ...
        <div className="desktop">
          ...
          <div className="desktop-icons">
            {TASKBAR_ICONS.map((ic) => (
                ...
                {/* Snake game - Desktop only icon */}
                <div
                  key="snake"
                  className="desktop-icon"
                  onDoubleClick={() => {
                    setOpenWindows(p => p.includes("snake") ? p : [...p, "snake"]);
                    setMinimized(p => p.filter(w => w !== "snake"));
                    setFocused("snake");
                  }}
                >
          ...
          <div className="taskbar">
            <div className="taskbar-left">
              <div style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "var(--accent)", letterSpacing: "0.1em" }}>lorenzOS</div>
            </div>
